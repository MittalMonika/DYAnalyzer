...
for(mu=muColl.begin(); mu!=muColl.end(); mu++){
  ...
  patMuonsegmentCompatibility_.push_back(mu->segmentCompability());
  patMuonchi2LocalPosition_.push_back(mu->combinedQuality().chi2LocalPosition);
  patMuontrkKink_.push_bach(mu->combinedQuality().trkKink);
  ...
  if(trkmu.isNull()){
    ...
    patMuonMatches_.push_back(trkmu->numberOfMatchedStations());
